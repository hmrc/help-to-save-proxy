...
class NSIConnectorSpec extends TestSupport with MockFactory with GeneratorDrivenPropertyChecks with MockPagerDuty {
  ...
  "the updateEmail method" must {
    "return a Right when the status is OK" in {
      mockPut(validNSIPayload, nsiCreateAccountUrl)(Right(HttpResponse(Status.OK)))
    ...
    "return a Left " when {
      "the status is not OK" in {
        forAll { status: Int ⇒
          whenever(status =!= Status.OK && status > 0) {
            inSequence {
              mockPut(validNSIPayload, nsiCreateAccountUrl)(Right(HttpResponse(status)))
      ...
      "the POST to NS&I fails" in {
        inSequence {
          mockPut(validNSIPayload, nsiCreateAccountUrl)(Left("Oh no!"))
          // WARNING: do not change the message in the following check - this needs to stay in line with the configuration in alert-config
  ...
  "the createAccount Method" must {
    ...
    "Return a SubmissionSuccess when the status is Created" in {
      mockPost(validNSIPayload, nsiCreateAccountUrl)(Right(HttpResponse(Status.CREATED, Some(Json.parse("""{"accountNumber" : "1234567890"}""")))))
    ...
    "Return a SubmissionSuccess when the status is CONFLICT" in {
      mockPost(validNSIPayload, nsiCreateAccountUrl)(Right(HttpResponse(Status.CONFLICT)))
      val result = nsiConnector.createAccount(validNSIPayload)
    ...
    "return a SubmissionFailure" when {
      "the status is BAD_REQUEST" in {
        ...
        inSequence {
          mockPost(validNSIPayload, nsiCreateAccountUrl)(Right(
      ...
      "the status is anything else" in {
        inSequence {
          mockPost(validNSIPayload, nsiCreateAccountUrl)(Right(HttpResponse(Status.BAD_GATEWAY)))
          // WARNING: do not change the message in the following check - this needs to stay in line with the configuration in alert-config
          mockPagerDutyAlert("Received unexpected http status in response to create account")
      ...
      "the call to createAccount fails" in {
        inSequence {
          mockPost(validNSIPayload, nsiCreateAccountUrl)(Left(""))
          // WARNING: do not change the message in the following check - this needs to stay in line with the configuration in alert-config
    ...
    "the health-check Method" must {
      "Return a Right when the status is OK" in {
        mockPut(validNSIPayload, nsiCreateAccountUrl, false)(Right(HttpResponse(Status.OK)))
      ...
      "Return a Left when the status is OK" in {
        forAll { status: Int ⇒
          whenever(status > 0 && status =!= Status.OK) {
            mockPut(validNSIPayload, nsiCreateAccountUrl, false)(Right(HttpResponse(status)))
            val result = nsiConnector.healthCheck(validNSIPayload)
            Await.result(result.value, 3.seconds).isLeft shouldBe true
          }
        }
