...
class NSIConnectorImpl @Inject() (auditConnector:    AuditConnector,
  ...
  override def createAccount(payload: NSIPayload)(implicit hc: HeaderCarrier, ec: ExecutionContext): EitherT[Future, SubmissionFailure, SubmissionSuccess] = {
    ...
    EitherT(httpProxy.post(nsiCreateAccountUrl, payload, Map(nsiAuthHeaderKey → nsiBasicAuth))(nsiPayloadFormat, hc.copy(authorization = None), ec)
  ...
  override def updateEmail(payload: NSIPayload)(implicit hc: HeaderCarrier, ec: ExecutionContext): Result[Unit] = EitherT[Future, String, Unit] {
    ...
    httpProxy.put(nsiCreateAccountUrl, payload, true, Map(nsiAuthHeaderKey → nsiBasicAuth))(nsiPayloadFormat, hc.copy(authorization = None), ec)
  ...
  override def healthCheck(payload: NSIPayload)(implicit hc: HeaderCarrier, ex: ExecutionContext): Result[Unit] = EitherT[Future, String, Unit] {
    httpProxy.put(nsiCreateAccountUrl, payload, false, Map(nsiAuthHeaderKey → nsiBasicAuth))(nsiPayloadFormat, hc.copy(authorization = None), ex)
      .map[Either[String, Unit]] { response ⇒
        response.status match {
